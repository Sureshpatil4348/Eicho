...
const ThirdStepComponent: React.FunctionComponent<{ token: string }> = ({ token }) => {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<AuthForgotPasswordSecondStepType>({ mode: 'all', resolver: yupResolver(formSchema) })

  const onSubmit = (data: AuthForgotPasswordSecondStepType): void => {
    setLoading(true)
    axios.post(API_URL.RESET_PASSWORD, { ...data, reset_token: token }).then((response) => {
      if (response.data) {
