...
const AccountGrowthChart = forwardRef((ref: any) => {
            ...
            return (
                ...
                >
                    <Typography variant="body2" color="text.secondary" fontWeight="medium">
                        {`Date: ${moment(payload?.[0]?.payload?.date).format('MMM DD, YYYY')}`}
                    </Typography>
                    {balanceData && (
    ...
    const calculateSummaryStats = () => {
        ...
        const totalDeposit = formatNumber(growthData?.totalDeposit || 0, "currency");
        const totalWithdraw = formatNumber(growthData?.totalWithdraw || 0, "currency");
        ...
        <>
                ...
                <div className="right">
                    {['1W', '1M', '3M', '1Y'].map((period) => (
                        <button
                            ...
                            className={(selectedPeriod === period && !customFilter) ? 'active' : ''}
                        >
                            {period}
                        </button>
                    ))}
                    <div className='custom_datepicker_main'>
                        <button
                            className={classNames('date_filter_btn form-control', { 'active': customFilter })}
                            onClick={() => setShowDatePicker(!showDatePicker)}
                        >
                            <FaRegCalendarAlt />
                            {dateRange[0].startDate ? format(dateRange[0].startDate, 'MMM dd, yyyy') : "Start Date"} - {dateRange[0].endDate ? format(dateRange[0].endDate, 'MMM dd, yyyy') : "End Date"}
                        </button>
                        {showDatePicker && (
                            <div className='date_range_picker custom_datepicker'>
                                ...
                                <div className='date_range_actions'>
                                    <button className='calendar_button calendar_button_border' onClick={handleCancelFilter}>
                                        Cancel
                                    </button>
            ...
            <div className='graph'>
                        ...
                        >
                            ...
                            >
                                ...
                                <ReferenceLine
                                    ...
                                    stroke="#666"
                                    strokeDasharray="2 2"
                                    label={{ value: "0%", position: "left", offset: 10, fontSize: 13 }}
                                />
                                ...
                                <Line
                                    ...
                                    activeDot={{ r: 6, stroke: '#050FD4', strokeWidth: 2, fill: 'white' }}
                                    name="Balance Growth"
                                    connectNulls={false}
                                />
                                ...
                                <Line
                                    ...
                                    activeDot={{ r: 6, stroke: '#1FCF43', strokeWidth: 2, fill: 'white' }}
                                    name="Equity Growth"
                                    connectNulls={false}
                                />
                            </LineChart>
                ...
                <div className="graph_button_sec">
                    <ul>
                        <li className={stats.totalGrowthPct.split('').includes('-') ? 'red' : 'green'}>
                            <div className="arrow">
                                {stats.totalGrowthPct.split('').includes('-') ? <img src="/images/growth-down.svg" alt="arrow-down" /> : <img src="/images/growth-icon.svg" alt="arrow-up" />}
                            </div>
                            <span>Total Growth in %</span>
                            <b>:</b>
                            <b>{stats.totalGrowthPct}%</b>
                        </li>
                        <li className={stats.sharpeRatio.split('').includes('-') ? 'red' : 'green'}>
                            <div className="arrow">
                                {stats.sharpeRatio.split('').includes('-') ? <img src="/images/growth-down.svg" alt="arrow-down" /> : <img src="/images/growth-icon.svg" alt="arrow-up" />}
                            </div>
                        ...
                        <li className='black'>
                            <div className="arrow">
                                <img src="/images/growth-normal.svg" alt="arrow-up-normal" />
                            </div>
                            <span>Total Deposit</span>
                            <b>:</b>
                            <b>{stats.totalDeposit}</b>
                        </li>
                        <li className={stats.totalWithdraw.split('').includes('-') ? 'red' : 'green'}>
                            <div className="arrow">
                                {stats.totalWithdraw.split('').includes('-') ? <img src="/images/growth-down.svg" alt="arrow-down" /> : <img src="/images/growth-icon.svg" alt="arrow-up" />}
                            </div>
