...
const ConfigUpdateModal: React.FC<{ closeModal: () => void; strategy_id: string }> = ({
  ...
  const getAllocationData = async () => {
    setFetchLoading(true)
    try {
      const res = await axios.get(API_URL.GET_SINGLE_ALLOCATION_DATA(strategy_id))
      if (res.data) {
  ...
  const handleSaveKeyPairs = async () => {
    ...
    setIsLoading(true)
    try {
      const res = await axios.post(API_URL.GET_SINGLE_ALLOCATION_DATA(strategy_id), payload)
  ...
  const handleAllocationChange = (index: number, field: 'percentage' | 'name', value: string | number) => {
    setAllocations((prev) => {
      const updated = [...prev]
          ...
          {allocations.map((a, i) => (
            ...
            >
              <TextField
                label="Key Pair Name"
                value={a.name}
                onChange={(e) => handleAllocationChange(i, 'name', e.target.value)}
                style={{ flex: 1 }}
              />
              <TextField
                label="Percentage (%)"
                  ...
                  <>
                    ...
                    {key === 'timeframe' ? (
                      <TextField
                        ...
                        fullWidth
                        value={value}
                        onChange={(e) => handleFieldChange(activeIndex, key, e.target.value)}
                      >
                      ...
                      <div style={{ display: 'flex', alignItems: 'center' }}>
                        <input
                          ...
                          checked={value}
                          onChange={(e) =>
                            handleFieldChange(activeIndex, key, e.target.checked)
                          }
                        />
